                        ...
                        <tbody>
                            <% 
                                try { 
                                    ...
                                    String updateSql = "UPDATE reservations SET status = 'Cancelled' WHERE status = 'Pending' AND check_out < CURDATE()";
                                    PreparedStatement updatePst = conn.prepareStatement(updateSql);
                                    updatePst.executeUpdate();
                                    updatePst.close();

                                    String searchQuery = request.getParameter("search");
                                    String query = "SELECT r.res_id, r.guest_name, r.room_type, r.check_in, r.check_out, r.total_bill, r.status, rm.room_number " +
                                                   "FROM reservations r LEFT JOIN rooms rm ON r.room_id = rm.room_id ";
                                    
                                    if (searchQuery != null && !searchQuery.trim().isEmpty()) {
                                        query += "WHERE r.guest_name LIKE ? ";
                                    }
                                    ...
                                    while (rs.next()) {
                                        ...
                                        if (roomNumber == null || roomNumber.equals("null")) roomNumber = "-";
                            ...
                            <tr>
                                ...
                                <td><%= guestName %></td>
                                <td><%= roomType %></td>
                                <td><span class="badge bg-dark"><%= roomNumber %></span></td>
                                <td><%= checkIn %></td>
                                <td><%= checkOut %></td>
